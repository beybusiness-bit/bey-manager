...
- **시각 자료 적극 활용**: 개발괴발에 새 개념을 추가할 때, 다이어그램·구조도·예시가 도움이 되는 내용이면 **반드시 ASCII 아트 또는 유니코드 박스 그림을 `<div class="dn-pre">` 안에 포함**한다.
  - 레이아웃 구조, UI 계층, 데이터 흐름, 순서도 등은 텍스트만으로는 이해가 어려움
  - ASCII 다이어그램 문자: `┌ ┐ └ ┘ │ ─ ├ ┤ ┬ ┴ ┼` 사용
  - 그림이 없어도 될 내용이면 생략 가능 (무조건 넣지는 않음)
- **퀴즈 자동 포함**: 새 챕터·섹션 추가 시 이해도 확인 퀴즈를 반드시 함께 추가
- **실제 베이 관리자 코드와 연결**: "베이 관리자에서는 ~" 식으로 배운 개념이 실제 코드에 어디 쓰이는지 언급하면 학습 효과 높아짐
...
### 커밋/푸시 플로우 — 2026-07-11 변경된 기본 동작

**⚠️ 매번 자동 푸시하지 않는다.** 사용자가 여러 수정을 연속 요청하므로, 커밋만 쌓고 사용자가 "푸시해줘"라고 할 때만 푸시한다.

1. 사용자가 수정 요청
2. Claude가 파일 수정
3. JS 문법 검증 (`node --check`)
4. **`git add` + `git commit` (로컬에만 저장)**
5. 사용자에게 커밋 완료 안내 ("커밋했어요, 더 수정할 것 있으면 말씀해주세요")

**푸시는 사용자가 명시적으로 "푸시해줘" / "배포해줘" / "확인해볼게" 라고 할 때만 수행한다.**

#### 푸시 방법 (우선순위 순)
```bash
# 1순위: 로컬 프록시 (세션이 살아있으면 이게 가장 간단)
git push origin main

# 2순위: push.sh (토큰 파일이 있을 때)
sh /home/user/bey-manager/.git/push.sh origin main

# 3순위: MCP push_files (위 둘 다 안 될 때 — 주의사항 아래 참고)
```

#### mcp__github__push_files 사용 시 필수 주의사항
- content 파라미터에 `$(cat file)` 같은 셸 문법 절대 사용 금지 — JSON 파라미터 안에서는 실행되지 않고 문자열로 들어가 파일이 깨짐
- 반드시 Read 도구로 파일 내용을 먼저 읽은 뒤, 그 내용을 그대로 content에 넣어야 함
- 이 실수가 2026-07-11에 실제로 발생해 GitHub의 app.js가 9,869줄 → 1줄로 깨진 선례 있음
...
- [ ] 17단계: 홈 대시보드 위젯 ⏸ **잠정 중단** (2026-04-26, 베이님 요청)
- [x] 18단계: 습관(✅) 페이지 구현 ✅
  - 오늘 뷰: 날짜 이동 + 습관 체크(달성/미달성 SVG) + 메모 버튼
  - 주별 뷰: 습관 × 7일 그리드 + 연속달성 표시
  - 월별 뷰: 캘린더 + work-dot-lg 방식 달성 dot
  - 습관 목록 탭: 그리드 카드 + 검색/정렬/페이저/전체선택/수정/삭제
  - 습관 CRUD: 이모지+이름+요일선택+색상 모달
  - 메모 기능: 날짜별 메모 저장 + 메모 목록 페이지네이션 모달
  - streak(연속달성) 계산
  - 테스트 데이터 주입 페이지: `habit-test-data.html`
