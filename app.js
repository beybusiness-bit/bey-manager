    ...
    const DEFAULT_MENUS = [
      { id: 'home', type: 'page', icon: '🏠', name: '홈 대시보드', slug: 'home', order: 0 },
      {
        ...
        children: [
          ...
          { id: 'business-tracker', type: 'page', icon: '💰', name: '돈을 벌자', slug: 'business-tracker', order: 2 },
        ]
      },
      {
    ...
    // Google Sign-In 초기화
    // Firebase Google 로그인
    // 전략: signInWithPopup 먼저 시도 → 팝업 차단 시 signInWithRedirect 자동 전환
    // (UA 기반 분기 제거 — iOS ITP 쿠키 차단 문제로 redirect-only 방식이 불안정)
    ...
    function handleFirebaseGoogleLogin() {
      ...
      if (btn) { btn.disabled = true; btn.innerHTML = '로그인 중...'; }
      ...
      firebase.auth().signInWithPopup(provider)
        ...
        .catch(function(e) {
          if (e.code === 'auth/popup-blocked' || e.code === 'auth/cancelled-popup-request') {
            /* 팝업 차단 → 리다이렉트 방식으로 자동 전환 */
            if (btn) { btn.disabled = true; btn.innerHTML = '로그인 페이지로 이동 중...'; }
            firebase.auth().signInWithRedirect(provider).catch(function(e2) {
              showAlert('로그인 실패 (' + (e2.code || '') + ')', e2.message || '');
              restore();
            });
          } else if (e.code === 'auth/popup-closed-by-user') {
            restore(); /* 사용자가 팝업 직접 닫음 — 조용히 복원 */
          } else {
            showAlert('로그인 실패 (' + (e.code || '') + ')', e.message || '');
            restore();
          }
        });
    }
    ...
    function initLogin() {
      ...
      /* 리다이렉트 로그인 결과 처리 (Google → 앱으로 복귀 후) */
      ...
      if (window.firebase && firebase.auth) {
        firebase.auth().getRedirectResult().then(function(result) {
          ...
          /* 에러를 사용자에게 노출해 진단 가능하게 함 */
          if (lc) lc.style.display = 'flex';
          if (e.code && e.code !== 'auth/no-auth-event') {
            showAlert('로그인 오류 (' + e.code + ')', e.message + '<br><br>이 오류 내용을 알려주세요.');
          }
        });
      } else {
        if (lc) lc.style.display = 'flex';
    ...
    // 돈을 벌자 — 매출 트래커
    // ========================================
    var BT_DEFAULT_CONFIG = {
      categories: [
        { id: 'btcat-1', name: '작업공간-일반', icon: '🪑', color: '#4CAF50', unitLabel: '이용자', defaultMarginRate: 0.91 },
        { id: 'btcat-2', name: '작업공간-행사', icon: '🎪', color: '#2196F3', unitLabel: '이용자', defaultMarginRate: 0.91 },
        { id: 'btcat-3', name: '모임/행사', icon: '🤝', color: '#9C27B0', unitLabel: '참가자', defaultMarginRate: null },
        { id: 'btcat-4', name: '소품샵', icon: '🛍️', color: '#FF9800', unitLabel: '구매건수', defaultMarginRate: 0.27 }
      ],
      fixedCosts: [
        { id: 'btfc-1', name: '임대료', amount: 1650000 },
        { id: 'btfc-2', name: '관리비(공과금)', amount: 100000 },
        { id: 'btfc-3', name: '와이파이+CCTV', amount: 82000 },
        { id: 'btfc-4', name: '비품비', amount: 50000 },
        { id: 'btfc-5', name: '소품샵 유지관리비', amount: 30000 }
      ],
      quarters: [
        { id: 'btq-1', name: '2026 3분기', startDate: '2026-07-01', endDate: '2026-09-30', monthlyTargetRevenue: 2310000 },
        { id: 'btq-2', name: '2026 4분기', startDate: '2026-10-01', endDate: '2026-12-31', monthlyTargetRevenue: 5120000 },
        { id: 'btq-3', name: '2027 1분기', startDate: '2027-01-01', endDate: '2027-03-31', monthlyTargetRevenue: 11760000 }
      ]
    };
    var btConfig = null;
    var btDailyEntries = [];  // [{date, items:[{id,catId,units,unitPrice,unitCost}]}]
    var btActions = [];       // [{id,name,catId,targetUnits,unitPrice,unitCost,date}]
    var btCurrentTab = 'dashboard';
    var btDashPeriod = 'month';   // 'day' | 'week' | 'month'
    var btInputDate = today();
    var btAddingItem = false;
    var btEditItemDate = null;
    ...
    var btCatIconTemp = '';
    var btAddingCost = false;
    var btEditCostId = null;
    var btAddingQuarter = false;
    var btEditQuarterId = null;
    var btDailyPeriod = 'day';   // 'day'|'week'|'month'|'quarter'
    ...
    function btPeriodRange(period, offset) {
      ...
      if (period === 'day') {
        var d = new Date(t); d.setDate(d.getDate() + (offset||0));
        var ds = fmt(d);
        return { start: ds, end: ds, label: ds, days: 1 };
      }
      if (period === 'week') {
        var d = new Date(t);
        var dow = (d.getDay() + 6) % 7;
        var mon = new Date(d); mon.setDate(d.getDate() - dow + (offset||0)*7);
        var sun = new Date(mon); sun.setDate(mon.getDate() + 6);
        var wLabel = ['일','월','화','수','목','금','토'];
        return { start: fmt(mon), end: fmt(sun), label: fmt(mon).slice(5)+'~'+fmt(sun).slice(5), days: 7, wLabel: wLabel };
      }
      if (period === 'month') {
        var d = new Date(t);
        var year = d.getFullYear(), month = d.getMonth() + (offset||0);
      ...
      if (allQs.length > 0) {
        ...
        return { start: qObj.startDate, end: qObj.endDate, label: qObj.name, days: 90, quarterTarget: (qObj.monthlyTargetRevenue||0)*3, quarterObj: qObj, qIdx: targetQIdx, qTotal: allQs.length };
      }
      // 분기 미설정 시 현재 달력 분기
    ...
    // 아이템 매출 계산 (units × unitPrice, 구형 revenue 필드 호환)
    function btItemRevenue(item) {
      if (item.unitPrice !== undefined && item.unitPrice !== null && item.unitPrice !== '') {
        return (Number(item.units) || 0) * (Number(item.unitPrice) || 0);
      }
    ...
    function btItemMargin(item) { return btItemRevenue(item) - btItemCost(item); }
    // 액션 목표 매출
    function btActionRevenue(a) { return (Number(a.targetUnits) || 0) * (Number(a.unitPrice) || 0); }
    function btActionMargin(a) { return btActionRevenue(a) - (Number(a.targetUnits) || 0) * (Number(a.unitCost) || 0); }

    function btFixedCostTotal() {
      if (!btConfig || !btConfig.fixedCosts) return 0;
      return btConfig.fixedCosts.reduce(function(s, fc) { return s + (Number(fc.amount) || 0); }, 0);
    }
    function btCalcPeriod(entries) {
      var revenue = 0, cost = 0, units = 0;
      entries.forEach(function(e) {
        (e.items || []).forEach(function(item) {
          revenue += btItemRevenue(item);
          cost    += btItemCost(item);
          units   += Number(item.units) || 0;
        });
      });
      return { revenue: revenue, cost: cost, margin: revenue - cost, units: units };
    }
    function btCalcByCategory(entries) {
      var map = {};
      entries.forEach(function(e) {
        (e.items || []).forEach(function(item) {
          if (!map[item.catId]) map[item.catId] = { revenue: 0, cost: 0, margin: 0, units: 0 };
          map[item.catId].revenue += btItemRevenue(item);
          map[item.catId].cost    += btItemCost(item);
          map[item.catId].margin  += btItemMargin(item);
          map[item.catId].units   += Number(item.units) || 0;
        });
      });
      return map;
    }
    function btActionsByCategory() {
      var map = {};
      (btActions || []).forEach(function(a) {
    ...
    function btGetCurrentQuarter() {
      var t = today();
      if (!btConfig || !btConfig.quarters) return null;
      return btConfig.quarters.find(function(q) { return q.startDate <= t && q.endDate >= t; }) || null;
    }
    function btEntriesInRange(start, end) {
      return btDailyEntries.filter(function(e) { return e.date >= start && e.date <= end; });
    }
    function _btElapsedMonths(startDate, todayStr) {
      var s = new Date(startDate), t = new Date(todayStr);
      var diff = (t.getFullYear() - s.getFullYear()) * 12 + (t.getMonth() - s.getMonth());
      return Math.max(1, diff + 1);
    }
    // 현재 대시보드 기간 범위 반환
    function btDashRange() {
      var t = today();
      if (btDashPeriod === 'day') return { start: t, end: t, label: '오늘', days: 1 };
      if (btDashPeriod === 'week') {
        var d = new Date(t);
        var dow = d.getDay(); // 0=일
        var mon = new Date(d); mon.setDate(d.getDate() - ((dow + 6) % 7));
        var sun = new Date(mon); sun.setDate(mon.getDate() + 6);
        var fmt = function(x) { return x.getFullYear() + '-' + String(x.getMonth()+1).padStart(2,'0') + '-' + String(x.getDate()).padStart(2,'0'); };
        return { start: fmt(mon), end: fmt(sun), label: '이번 주', days: 7 };
      }
    ...
    function btSvgBarChart(rangeStart, rangeEnd) {
      ...
      var eDate = new Date(rangeEnd);
      while (d <= eDate && days.length < 31) {
    ...
    function loadBt() {
      try {
        var saved = localStorage.getItem('btConfig');
        btConfig = saved ? JSON.parse(saved) : JSON.parse(JSON.stringify(BT_DEFAULT_CONFIG));
      } catch(e) { btConfig = JSON.parse(JSON.stringify(BT_DEFAULT_CONFIG)); }
      try { btDailyEntries = JSON.parse(localStorage.getItem('btDailyEntries') || '[]'); } catch(e) { btDailyEntries = []; }
      if (!Array.isArray(btDailyEntries)) btDailyEntries = [];
      try { btActions = JSON.parse(localStorage.getItem('btActions') || '[]'); } catch(e) { btActions = []; }
      if (!Array.isArray(btActions)) btActions = [];
    }
    function saveBt() {
      localStorage.setItem('btConfig', JSON.stringify(btConfig));
      localStorage.setItem('btDailyEntries', JSON.stringify(btDailyEntries));
      localStorage.setItem('btActions', JSON.stringify(btActions));
      if (window.FS && FS.isConnected()) FS.sync(['돈을벌자']);
    }
    function saveBtDailyEntries() {
      localStorage.setItem('btDailyEntries', JSON.stringify(btDailyEntries));
      if (window.FS && FS.isConnected()) FS.sync(['돈을벌자']);
    }

    function renderBusinessTrackerPage() {
      loadBt();
      var container = document.getElementById('businessTrackerContent');
      if (!container) return;
      if (!btInputDate) btInputDate = today();
      var tabs = [
        { id: 'dashboard', label: '📊 대시보드' },
        { id: 'daily',     label: '📅 달성 내용' },
        { id: 'planning',  label: '📋 액션플랜' },
        { id: 'quarters',  label: '🎯 분기별 목표' },
        { id: 'settings',  label: '⚙️ 설정' }
      ];
      var h = '<div class="tab-nav" id="btTabNav">';
      tabs.forEach(function(t) {
        h += '<button class="tab-btn' + (btCurrentTab === t.id ? ' active' : '') + '" onclick="switchBtTab(\'' + t.id + '\')">' + t.label + '</button>';
      });
      h += '</div>';
      h += '<div id="btTabContent"></div>';
    ...
    function switchBtTab(tab) {
      ...
      btAddingCost = false; btEditCostId = null;
      btAddingQuarter = false; btEditQuarterId = null;
      document.querySelectorAll('#btTabNav .tab-btn').forEach(function(btn, i) {
        btn.classList.toggle('active', i === ['dashboard','daily','planning','quarters','settings'].indexOf(tab));
      });
      btRenderCurrentTab();
    }

    function btRenderCurrentTab() {
      var el = document.getElementById('btTabContent');
      if (!el) return;
      if (btCurrentTab === 'dashboard') el.innerHTML = btBuildDashboard();
      else if (btCurrentTab === 'daily')  el.innerHTML = btBuildDaily();
      else if (btCurrentTab === 'planning') el.innerHTML = btBuildPlanning();
      else if (btCurrentTab === 'quarters') el.innerHTML = btBuildQuarters();
      else if (btCurrentTab === 'settings') el.innerHTML = btBuildSettings();
    }

    // ── 대시보드 ──
    function btBuildDashboard() {
      var range = btDashRange();
      var entries = btEntriesInRange(range.start, range.end);
      var calc = btCalcPeriod(entries);
      var fc = btFixedCostTotal();
      // 고정비 배분: 기간 비율로
      var fcProrated = Math.round(fc / 30 * range.days);
      var q = btGetCurrentQuarter();
      var mTarget = q ? (Number(q.monthlyTargetRevenue) || 0) : 0;
      var periodTarget = Math.round(mTarget / 30 * range.days);

      var h = '';

      // 기간 탭
      h += '<div class="bt-period-tabs">';
      ['day','week','month'].forEach(function(p, i) {
        var labels = ['오늘','이번 주','이번 달'];
        h += '<button class="bt-period-btn' + (btDashPeriod === p ? ' active' : '') + '" onclick="btSwitchDashPeriod(\'' + p + '\')">' + labels[i] + '</button>';
      });
      h += '</div>';
        ...
        cats.forEach(function(cat) {
          ...
          if (catActions.length) {
            ...
            catActions.slice(0, 2).forEach(function(a) {
              h += '<div class="bt-compare-action-chip">' + escapeHtml(a.name) + (a.date ? ' (' + a.date.slice(5) + ')' : '') + '</div>';
            });
      ...
      // 카테고리 없는 액션 (기타)
      var noCatActions = (actionsByCat['__none'] || []).filter(function(a) {
        return !a.date || (a.date >= range.start && a.date <= range.end);
      });
      if (noCatActions.length) {
        h += '<div class="bt-section-title" style="margin-top:16px;">기타 액션</div>';
        h += '<div class="bt-recent-list">';
        noCatActions.forEach(function(a) {
          h += '<div class="bt-recent-row">' + escapeHtml(a.name) + (a.date ? ' — ' + a.date.slice(5) : '') + '</div>';
        });
        h += '</div>';
      }
    ...
    function btSwitchDashPeriod(p) {
      btDashPeriod = p;
      btRenderCurrentTab();
    }
    function btKpiCard(label, val, cls) {
      return '<div class="bt-kpi-card"><div class="bt-kpi-val' + (cls ? ' ' + cls : '') + '">' + val + '</div><div class="bt-kpi-label">' + label + '</div></div>';
    }

    // ── 달성 내용 (period-aware) ──
    function btBuildDaily() {
      var cats = (btConfig && btConfig.categories) || [];
      var h = '';

      // 기간 탭 (일/주/월/분기 — 뷰 방식 직접 결정)
      h += '<div class="bt-period-tabs">';
      ...
      if (btDailyPeriod === 'day') btInputDate = range.start;
      ...
      h += '<div class="bt-date-label-wrap"><button class="bt-date-label-btn">' + range.label + ' <span class="bt-cal-icon">📅</span></button>';
      ...
      (function() {
        ...
        if (btDailyPeriod === 'quarter') {
          _target = range.quarterTarget || 0;
        } else {
          var _qs2 = (btConfig && btConfig.quarters) || [];
          var _q2 = _qs2.find(function(q) { return q.startDate <= _rs && q.endDate >= _rs; });
          if (!_q2) _q2 = _qs2.find(function(q) { return q.startDate <= _re && q.endDate >= _re; });
          var _mt = _q2 ? (Number(_q2.monthlyTargetRevenue) || 0) : 0;
          if (btDailyPeriod === 'day') {
            if (_mt > 0) { var _dim = new Date((new Date(_rs+'T00:00:00')).getFullYear(), (new Date(_rs+'T00:00:00')).getMonth()+1, 0).getDate(); _target = Math.round(_mt / _dim); }
          } else {
            if (_mt > 0) _target = Math.round(_mt / 30 * (range.days || 7));
          }
        }
        if (!_target) return;
        var _pct = Math.min(200, Math.round(_dSumCalc.revenue / _target * 100));
        var _tpct = btTimePct(_rs, _re);
        h += '<div class="bt-goal-row">';
        h += '<div class="bt-goal-label">기간 목표 대비<span class="bt-goal-pct' + (_pct >= 100 ? ' hit' : '') + '">' + _pct + '%</span></div>';
        h += '<div class="bt-goal-sub">' + btFmtW(_dSumCalc.revenue) + ' / 목표 ' + btFmtW(_target);
        var _gap = _target - _dSumCalc.revenue;
        if (_gap > 0) h += ' <span style="color:var(--text-secondary,#888);font-size:11px;">— 부족 ' + btFmtW(_gap) + '</span>';
        else h += ' <span style="color:#1a8c60;font-size:11px;">— 초과 ' + btFmtW(-_gap) + '</span>';
        h += '</div>';
        h += '<div class="bt-dual-bar">';
        h += '<div class="bt-dual-bar-row"><span class="bt-dual-label">달성</span><div class="bt-progress-wrap bt-dual-prog"><div class="bt-progress-bar' + (_pct >= 100 ? ' full' : '') + '" style="width:' + Math.min(100,_pct) + '%"></div></div><span class="bt-dual-pct">' + _pct + '%</span></div>';
      ...
      if (btDailyPeriod === 'day') {
        ...
        if (cats.length === 0 && items.length === 0) {
          ...
          h += '<div class="bt-empty">이 날짜의 달성 내용이 없습니다.</div>';
        } else {
          h += '<div class="bt-item-grid">';
          items.forEach(function(item, sortedIdx) {
            var realIdx = (dateEntry.items || []).indexOf(item);
            if (btEditItemDate === btInputDate && btEditItemIdx === realIdx)
              h += '<div class="bt-item-grid-span">' + btBuildItemForm(item, realIdx, btInputDate) + '</div>';
            else h += btBuildItemCard(item, realIdx);
          });
          h += '</div>';
        }
      ...
      // ─ 주 뷰 — 7컬럼 그리드 ─
        ...
        h += '<div class="bt-week-grid">';
        for (var wi = 0; wi < 7; wi++) {
          var wd = new Date(range.start + 'T00:00:00');
          wd.setDate(wd.getDate() + wi);
          var wds = wd.getFullYear()+'-'+String(wd.getMonth()+1).padStart(2,'0')+'-'+String(wd.getDate()).padStart(2,'0');
          var wIsToday = wds === today();
          var wIsWeekend = wi >= 5;
          var wEntry = btDailyEntries.find(function(e) { return e.date === wds; });
          var wItems = wEntry ? (wEntry.items || []).slice().sort(function(a,b){ var ak=a.time||'zz',bk=b.time||'zz'; return ak<bk?-1:ak>bk?1:0; }) : [];
          h += '<div class="bt-week-col' + (wIsToday?' is-today':'') + (wIsWeekend?' weekend':'') + '">';
          h += '<div class="bt-week-col-head" onclick="btNavToDay(\'' + wds + '\')">';
          h += '<span class="bt-week-day-name">' + dayNames[wi] + '</span>';
          h += '<span class="bt-week-day-num' + (wIsToday?' today-num':'') + '">' + wd.getDate() + '</span>';
          h += '</div>';
          wItems.forEach(function(it) {
            var realIdx = (wEntry.items || []).indexOf(it);
            var cat = cats.find(function(c) { return c.id === it.catId; });
            var catColor = (cat && cat.color) ? cat.color : '#ddd';
            if (btEditItemDate === wds && btEditItemIdx === realIdx) {
              h += '<div class="bt-week-item-edit">' + btBuildItemForm(it, realIdx, wds) + '</div>';
            } else {
              h += '<div class="bt-week-item" style="border-left:3px solid ' + catColor + ';background:' + catColor + '15;" onclick="btShowItemDetail(\'' + wds + '\',' + realIdx + ')">';
      ...
      // ─ 분기 뷰 — 월별 섹션 (데스크탑 가로 3열) ─
        ...
        while (new Date(qYr, qMo, 1) <= qEnd) {
          ...
          var moLastStr = qYr+'-'+String(qMo+1).padStart(2,'0')+'-'+String(moLast.getDate()).padStart(2,'0');
          var moEntries = btEntriesInRange(moFirstStr, moLastStr);
          var moCalc = btCalcPeriod(moEntries);
          h += '<div class="bt-quarter-month-sec">';
          h += '<div class="bt-quarter-month-head"><span>' + qYr + '년 ' + monthNames[qMo] + '</span>';
          if (moCalc.revenue) h += '<span class="bt-quarter-month-sum">' + btFmtW(moCalc.revenue) + '</span>';
          h += '</div>';
          if (moEntries.length === 0) {
            h += '<div class="bt-empty" style="font-size:12px;padding:8px 0;">기록 없음</div>';
          } else {
            moEntries.slice().sort(function(a,b){ return a.date>b.date?1:-1; }).forEach(function(entry) {
              var dayItems = (entry.items || []).slice().sort(function(a,b){ var ak=a.time||'zz',bk=b.time||'zz'; return ak<bk?-1:ak>bk?1:0; });
              if (!dayItems.length) return;
              h += '<div class="bt-qtr-day-row">';
              h += '<div class="bt-qtr-day-label" onclick="btNavToDay(\'' + entry.date + '\')">' + entry.date.slice(5).replace('-','/') + '</div>';
              h += '<div class="bt-qtr-day-items">';
              dayItems.forEach(function(it, si) {
                var realIdx = (entry.items || []).indexOf(it);
                var cat = cats.find(function(c) { return c.id === it.catId; });
                var catColor = (cat && cat.color) ? cat.color : '#ddd';
                if (btEditItemDate === entry.date && btEditItemIdx === realIdx) {
                  h += '<div>' + btBuildItemForm(it, realIdx, entry.date) + '</div>';
                } else {
    ...
    // 날짜 헤더/라벨 클릭 → 해당 날짜 일뷰로 이동 (offset 리셋 없이)
    function btNavToDay(dateStr) {
      btDailyPeriod = 'day';
      btDailyOffset = _btDayOffset(dateStr);
      btInputDate = dateStr;
      btAddingItem = false; btEditItemDate = null; btEditItemIdx = null;
      btRenderCurrentTab();
    }
    function btActionNavToDay(dateStr) {
      btActionPeriod = 'day';
      btActionOffset = _btDayOffset(dateStr);
      btAddingAction = false; btEditActionId = null;
      btRenderCurrentTab();
    }
    ...
    // ── 달성내용 상세보기 모달 ──
    function btShowItemDetail(date, idx) {
      var entry = btDailyEntries.find(function(e) { return e.date === date; });
      if (!entry) return;
      var item = (entry.items || [])[idx];
      if (!item) return;
      var cats = (btConfig && btConfig.categories) || [];
      var cat = cats.find(function(c) { return c.id === item.catId; });
      var rev = btItemRevenue(item);
      var cst = btItemCost(item);
      var margin = rev - cst;
      var mRate = rev > 0 ? Math.round(margin / rev * 100) : 0;
      var catColor = (cat && cat.color) ? cat.color : '#ddd';
      var el = document.getElementById('btDetailModal');
      if (!el) {
        el = document.createElement('div'); el.id = 'btDetailModal';
        el.className = 'bt-detail-overlay'; document.body.appendChild(el);
      }
      el.innerHTML = '<div class="bt-detail-box">' +
        '<div class="bt-detail-header" style="border-left:4px solid ' + catColor + '">' +
        '<span class="bt-detail-title">' + (cat ? cat.icon + ' ' + escapeHtml(cat.name) : '📦 매출') + '</span>' +
        '<button class="bt-detail-close" onclick="btCloseDetail()">✕</button>' +
        '</div>' +
        '<div class="bt-detail-body">' +
        '<div class="bt-detail-row"><span class="bt-detail-label">날짜</span><span>' + date + (item.time ? '  ' + item.time : '') + '</span></div>' +
        (item.units ? '<div class="bt-detail-row"><span class="bt-detail-label">건수</span><span>' + btFmtN(item.units) + (cat ? cat.unitLabel || '건' : '건') + '</span></div>' : '') +
        (item.unitPrice ? '<div class="bt-detail-row"><span class="bt-detail-label">객단가</span><span>' + btFmtW(item.unitPrice) + '</span></div>' : '') +
        '<div class="bt-detail-row"><span class="bt-detail-label">매출</span><span style="font-weight:700;font-size:15px;">' + btFmtW(rev) + '</span></div>' +
        (cst ? '<div class="bt-detail-row"><span class="bt-detail-label">원가</span><span>' + btFmtW(cst) + '</span></div>' : '') +
        (rev ? '<div class="bt-detail-row"><span class="bt-detail-label">수익</span><span style="color:' + (margin >= 0 ? '#1a8c60' : '#e53935') + ';font-weight:600;">' + btFmtW(margin) + ' <span style="font-size:11px;font-weight:400;">(' + mRate + '%)</span></span></div>' : '') +
        (item.memo ? '<div class="bt-detail-row bt-detail-memo"><span class="bt-detail-label">메모</span><span>' + escapeHtml(item.memo) + '</span></div>' : '') +
        '</div>' +
        '<div class="bt-detail-footer">' +
        '<button class="btn-confirm" onclick="btCloseDetail();btEditItemInDay(\'' + date + '\',' + idx + ')">✏️ 수정</button>' +
        '<button class="btn-cancel" onclick="btCloseDetail()">닫기</button>' +
        '</div>' +
        '</div>';
      el.style.display = 'flex';
      el.onclick = function(e) { if (e.target === el) btCloseDetail(); };
    }
    function btCloseDetail() {
      var el = document.getElementById('btDetailModal');
      if (el) el.style.display = 'none';
    }

    // ── 액션플랜 상세보기 모달 ──
    function btShowActionDetail(id) {
      var action = btActions.find(function(a) { return a.id === id; });
      if (!action) return;
      var cats = (btConfig && btConfig.categories) || [];
      var cat = cats.find(function(c) { return c.id === action.catId; });
      var planRev = btActionRevenue(action);
      var planCst = btActionMargin ? (planRev - btActionMargin(action)) : 0;
      var planMargin = btActionMargin ? btActionMargin(action) : 0;
      var mRate = planRev > 0 ? Math.round(planMargin / planRev * 100) : 0;
      var catColor = (cat && cat.color) ? cat.color : '#ddd';
      var el = document.getElementById('btDetailModal');
      if (!el) {
        el = document.createElement('div'); el.id = 'btDetailModal';
      ...
      el.innerHTML = '<div class="bt-detail-box">' +
        '<div class="bt-detail-header" style="border-left:4px solid ' + catColor + '">' +
        '<span class="bt-detail-title">' + (cat ? cat.icon + ' ' : '📋 ') + escapeHtml(action.name) + '</span>' +
        '<button class="bt-detail-close" onclick="btCloseDetail()">✕</button>' +
        '</div>' +
        '<div class="bt-detail-body">' +
        (cat ? '<div class="bt-detail-row"><span class="bt-detail-label">카테고리</span><span>' + escapeHtml(cat.name) + '</span></div>' : '') +
        (action.date ? '<div class="bt-detail-row"><span class="bt-detail-label">날짜</span><span>' + action.date + (action.time ? '  ' + action.time : '') + '</span></div>' : '') +
        (action.targetUnits ? '<div class="bt-detail-row"><span class="bt-detail-label">목표 건수</span><span>' + btFmtN(action.targetUnits) + (cat ? cat.unitLabel || '건' : '건') + '</span></div>' : '') +
        (action.unitPrice ? '<div class="bt-detail-row"><span class="bt-detail-label">객단가</span><span>' + btFmtW(action.unitPrice) + '</span></div>' : '') +
        (planRev ? '<div class="bt-detail-row"><span class="bt-detail-label">목표 매출</span><span style="font-weight:700;font-size:15px;">' + btFmtW(planRev) + '</span></div>' : '') +
    ...
    function btBuildItemCard(item, idx, date) {
      ...
      var cst = btItemCost(item);
      var margin = rev - cst;
      var units = Number(item.units) || 0;
      var unitPrice = item.unitPrice !== undefined ? Number(item.unitPrice) : (units > 0 ? Math.round(rev / units) : 0);
      ...
      h += '<div class="bt-num-row"><span class="bt-num-label">객단가</span><span class="bt-num-val">' + btFmtW(unitPrice) + '</span></div>';
      h += '<div class="bt-num-row"><span class="bt-num-label">단위원가</span><span class="bt-num-val">' + btFmtW(unitCost) + '</span></div>';
      h += '</div>';
    ...
    function btBuildItemForm(item, idx, cardDate) {
      ...
      h += '<input type="date" id="btFormDate" class="bt-input" max="' + today() + '" value="' + defDate + '">';
      h += '</div>';
      h += '<div class="bt-form-row"><label>시간 <span style="font-weight:400;color:var(--text-secondary,#999);font-size:11px;">(선택)</span></label>';
      h += '<input type="time" id="btFormTime" class="bt-input" value="' + defTime + '">';
      h += '</div>';
      h += '</div>';
      ...
      cats.forEach(function(cat) {
        h += '<option value="' + cat.id + '"' + (selCat === cat.id ? ' selected' : '') + '>' + cat.icon + ' ' + escapeHtml(cat.name) + '</option>';
      });
      h += '</select></div>';
      ...
      h += '</div>';
      h += '<div class="bt-form-row"><label>자동 계산 결과</label>';
      h += '<div id="btAutoCalcResult" class="bt-auto-calc">건수 × 가격 입력 시 자동 계산</div>';
      h += '</div>';
      h += '</div>';
    ...
    function btAutoCalcDisplay() {
      var units = parseInt(document.getElementById('btFormUnits') ? document.getElementById('btFormUnits').value : '0', 10) || 0;
      var uprice = parseInt(document.getElementById('btFormUnitPrice') ? document.getElementById('btFormUnitPrice').value : '0', 10) || 0;
      var ucost  = parseInt(document.getElementById('btFormUnitCost') ? document.getElementById('btFormUnitCost').value : '0', 10) || 0;
      var rev = units * uprice;
      var cst = units * ucost;
      // 단위 원가 없으면 카테고리 기본 마진율 사용
      if (!ucost) {
        var catEl = document.getElementById('btFormCat');
        if (catEl) {
          var cat = btConfig && btConfig.categories.find(function(c) { return c.id === catEl.value; });
          if (cat && cat.defaultMarginRate != null) cst = Math.round(rev * (1 - cat.defaultMarginRate));
        }
      }
      var margin = rev - cst;
    ...
    function btFormCatChange(catId) {
      var cats = (btConfig && btConfig.categories) || [];
      var cat = cats.find(function(c) { return c.id === catId; });
      var unitLabel = cat ? cat.unitLabel : '건';
      var labelEl = document.querySelector('#btFormUnits') ? document.querySelector('label[for]') : null;
      // unitLabel 업데이트 — 라벨 직접 변경
      var unitsRow = document.getElementById('btFormUnits') ? document.getElementById('btFormUnits').closest('.bt-form-row') : null;
      if (unitsRow) unitsRow.querySelector('label').textContent = '판매건수 (' + unitLabel + ')';
      // unitCost 기본값 힌트 업데이트
      var ucostEl = document.getElementById('btFormUnitCost');
      if (ucostEl && !ucostEl.value) {
        ucostEl.placeholder = cat && cat.defaultMarginRate != null ? '기본값 있음 (' + Math.round(cat.defaultMarginRate*100) + '% 마진율)' : '0원';
      }
      btAutoCalcDisplay();
    }
    ...
    function btSaveItem() {
      var catEl     = document.getElementById('btFormCat');
      var unitsEl   = document.getElementById('btFormUnits');
      var upriceEl  = document.getElementById('btFormUnitPrice');
      var ucostEl   = document.getElementById('btFormUnitCost');
      var memoEl    = document.getElementById('btFormMemo');
      if (!catEl || !unitsEl) return;
      var catId     = catEl.value;
      var units     = parseInt(unitsEl.value, 10) || 0;
      var unitPrice = parseInt(upriceEl ? upriceEl.value : '0', 10) || 0;
      var memo      = memoEl ? memoEl.value.trim() : '';
      var timeEl = document.getElementById('btFormTime');
      var itemTime = timeEl ? timeEl.value : '';
      var item = { id: btGenId('btitem'), catId: catId, units: units, unitPrice: unitPrice, time: itemTime, memo: memo };
      var ucostVal = ucostEl ? ucostEl.value : '';
      if (ucostVal !== '') {
        item.unitCost = parseInt(ucostVal, 10) || 0;
      }
      // 단위 원가 없으면 카테고리 기본 마진율로 계산
      if (item.unitCost === undefined) {
        var cat = btConfig && btConfig.categories.find(function(c) { return c.id === catId; });
        if (cat && cat.defaultMarginRate != null) {
          item.unitCost = Math.round(unitPrice * (1 - cat.defaultMarginRate));
        }
      }

      // 날짜: 폼의 날짜 필드 사용 (항상 존재)
      ...
      // 편집 시 날짜가 바뀌었으면 원래 날짜에서 제거 후 새 날짜에 추가
      if (btEditItemIdx !== null && btEditItemDate && btEditItemDate !== saveDate) {
        var oldEntryIdx = btDailyEntries.findIndex(function(e) { return e.date === btEditItemDate; });
        if (oldEntryIdx >= 0) {
          btDailyEntries[oldEntryIdx].items.splice(btEditItemIdx, 1);
          if (!btDailyEntries[oldEntryIdx].items.length) btDailyEntries.splice(oldEntryIdx, 1);
        }
    ...
    function btDeleteItem(date, idx) {
      showConfirm('삭제 확인', '이 항목을 삭제할까요?', function(ok) { if (!ok) return;
        ...
        if (!btDailyEntries[entryIdx].items.length) btDailyEntries.splice(entryIdx, 1);
        saveBtDailyEntries();
        btRenderCurrentTab();
        showToast('삭제됨');
      });
    }
    function btChangeDate(delta) {
      var d = new Date(btInputDate);
      d.setDate(d.getDate() + delta);
      btInputDate = d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0') + '-' + String(d.getDate()).padStart(2,'0');
      btAddingItem = false; btEditItemDate = null; btEditItemIdx = null;
      btRenderCurrentTab();
    }
    function btSetDate(val) {
      if (!val) return;
      btInputDate = val;
      btAddingItem = false; btEditItemDate = null; btEditItemIdx = null;
      if (btCurrentTab !== 'daily') { btCurrentTab = 'daily'; renderBusinessTrackerPage(); return; }
      btRenderCurrentTab();
    }

    // ── 액션플랜 ──
    function btBuildPlanning() {
      var cats = (btConfig && btConfig.categories) || [];
      var h = '';
      ...
      (function() {
        ...
        h += '<div class="bt-gap-row" style="margin-bottom:8px;"><span class="bt-gap-label">기간 목표 <strong>' + btFmtW(_periodTarget) + '</strong></span></div>';
        var rows = [
        ...
        rows.forEach(function(r) {
          h += '<div class="bt-dual-bar-row"><span class="bt-dual-label">' + r.label + '</span>';
          h += '<div class="bt-progress-wrap bt-dual-prog"><div class="' + r.barClass + r.hitClass + '" style="width:' + Math.min(100,r.pct) + '%"></div></div>';
          h += '<span class="bt-dual-pct' + (r.barClass==='bt-time-bar'?' bt-time-pct':(r.pct>=100?' hit':'')) + '">' + r.pct + '%</span></div>';
        });
      ...
      // ─ 일 뷰 ─
      if (btActionPeriod === 'day') {
        var dayActions = filtActions;
        if (cats.length === 0 && dayActions.length === 0) {
          h += '<div class="bt-empty">설정 탭에서 카테고리를 먼저 추가하세요.</div>';
        } else if (dayActions.length === 0) {
          h += '<div class="bt-empty">이 날짜의 액션플랜이 없습니다.</div>';
        } else {
          h += '<div class="bt-action-grid">';
          dayActions.forEach(function(action) {
      ...
      // ─ 주 뷰 — 7컬럼 그리드 ─
        ...
        h += '<div class="bt-week-grid">';
        for (var wi = 0; wi < 7; wi++) {
          var wd = new Date(aRange.start + 'T00:00:00');
          wd.setDate(wd.getDate() + wi);
          var wds = wd.getFullYear()+'-'+String(wd.getMonth()+1).padStart(2,'0')+'-'+String(wd.getDate()).padStart(2,'0');
          var wIsToday = wds === today();
          var wIsWeekend = wi >= 5;
          var wDayActions = btActions.filter(function(a){ return a.date === wds; }).slice().sort(function(a,b){
            var ak=a.time||'zz',bk=b.time||'zz'; return ak<bk?-1:ak>bk?1:0;
          });
          h += '<div class="bt-week-col' + (wIsToday?' is-today':'') + (wIsWeekend?' weekend':'') + '">';
          h += '<div class="bt-week-col-head" onclick="btActionNavToDay(\'' + wds + '\')">';
          h += '<span class="bt-week-day-name">' + dayNames[wi] + '</span>';
          h += '<span class="bt-week-day-num' + (wIsToday?' today-num':'') + '">' + wd.getDate() + '</span>';
          h += '</div>';
      ...
      // ─ 분기 뷰 — 월별 섹션 (데스크탑 가로 3열) ─
        ...
        while (new Date(qYr, qMo, 1) <= qEnd) {
          ...
          var moLastStr = qYr+'-'+String(qMo+1).padStart(2,'0')+'-'+String(moLastDate.getDate()).padStart(2,'0');
          var moActions = btActions.filter(function(a){ return a.date && a.date >= moFirstStr && a.date <= moLastStr; });
          var moRev = moActions.reduce(function(s,a){ return s+btActionRevenue(a); }, 0);
          h += '<div class="bt-quarter-month-sec">';
          h += '<div class="bt-quarter-month-head"><span>' + qYr + '년 ' + monthNames[qMo] + '</span>';
          if (moRev) h += '<span class="bt-quarter-month-sum">예상 ' + btFmtW(moRev) + '</span>';
          h += '</div>';
          if (moActions.length === 0) {
            h += '<div class="bt-empty" style="font-size:12px;padding:8px 0;">액션 없음</div>';
          } else {
            // 날짜별 그룹
            var daysInMo = [];
            moActions.forEach(function(a) { if (daysInMo.indexOf(a.date) < 0) daysInMo.push(a.date); });
            daysInMo.sort();
            daysInMo.forEach(function(dstr) {
              var dayActs = moActions.filter(function(a){ return a.date === dstr; }).slice().sort(function(a,b){
                var ak=a.time||'zz',bk=b.time||'zz'; return ak<bk?-1:ak>bk?1:0;
              });
              h += '<div class="bt-qtr-day-row">';
              h += '<div class="bt-qtr-day-label" onclick="btActionNavToDay(\'' + dstr + '\')">' + dstr.slice(5).replace('-','/') + '</div>';
              h += '<div class="bt-qtr-day-items">';
    ...
    function btBuildActionCard(action) {
      ...
      var planRev = btActionRevenue(action);
      var planMargin = btActionMargin(action);
      var h = '<div class="bt-action-card">';
    ...
    // ── 달성내용 캘린더뷰 ──
    function btBuildDailyCalendar(range) {
      var yr, mo;
      if (range && range.start) {
        var _d = new Date(range.start + 'T00:00:00');
        yr = _d.getFullYear(); mo = _d.getMonth() + 1;
      } else {
        var now = new Date();
      ...
      for (var day = 1; day <= daysInMonth; day++) {
        ...
        var entry = btDailyEntries.find(function(e) { return e.date === ds; });
        var dayItems = entry ? (entry.items || []).slice().sort(function(a,b){ var ak=a.time||'zz',bk=b.time||'zz'; return ak<bk?-1:ak>bk?1:0; }) : [];
        var hasItems = dayItems.length > 0;
        h += '<div class="work-cal-cell' + (isToday ? ' is-today' : '') + (isWeekend ? ' weekend' : '') + (hasItems ? ' has-items' : '') + '" onclick="btNavToDay(\'' + ds + '\')">';
        h += '<div class="work-cal-date">' + day + '</div>';
        if (hasItems) {
          h += '<div class="work-cal-items-desktop">';
          dayItems.slice(0, 3).forEach(function(it, _ci) {
            var realIdx = (entry.items || []).indexOf(it);
            var cat = cats.find(function(c) { return c.id === it.catId; });
            var dotColor = (cat && cat.color) ? cat.color : 'var(--primary-yellow)';
            var rev = btItemRevenue(it);
            h += '<div class="work-cal-item" onclick="event.stopPropagation();btShowItemDetail(\'' + ds + '\',' + realIdx + ')" style="cursor:pointer;">';
            ...
            h += '<span class="work-cal-item-title">' + (it.time ? it.time + ' ' : '') + escapeHtml((cat ? cat.name : '') || '매출') + ' ' + btFmtW(rev) + '</span>';
            h += '</div>';
          });
    ...
    // ── 액션플랜 캘린더뷰 ──
    function btBuildActionCalendar(range) {
      var yr, mo;
      if (range && range.start) {
        var _d = new Date(range.start + 'T00:00:00');
        yr = _d.getFullYear(); mo = _d.getMonth() + 1;
      ...
      for (var day = 1; day <= daysInMonth; day++) {
        ...
        var dow = (firstDayKR + day - 1) % 7;
        var isWeekend = (dow === 5 || dow === 6);
        var dayActions = btActions.filter(function(a) { return a.date === ds; }).slice().sort(function(x,y){
          var xk = x.time || '99:99', yk = y.time || '99:99'; return xk<yk?-1:xk>yk?1:0;
        });
        var hasItems = dayActions.length > 0;
        h += '<div class="work-cal-cell' + (isToday ? ' is-today' : '') + (isWeekend ? ' weekend' : '') + (hasItems ? ' has-items' : '') + '" onclick="btActionNavToDay(\'' + ds + '\')">';
        h += '<div class="work-cal-date">' + day + '</div>';
        if (hasItems) {
          h += '<div class="work-cal-items-desktop">';
          dayActions.slice(0, 3).forEach(function(a) {
            ...
            var dotColor = (cat && cat.color) ? cat.color : 'var(--primary-yellow)';
            h += '<div class="work-cal-item" onclick="event.stopPropagation();btShowActionDetail(\'' + a.id + '\')" style="cursor:pointer;">';
            h += '<span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:' + dotColor + ';flex-shrink:0;margin-right:3px;"></span>';
    ...
    // 시작일로부터 "YYYY N분기" 자동 생성
    function btQuarterAutoName(startDate) {
      if (!startDate) return '';
      var d = new Date(startDate + 'T00:00:00');
      var qNum = Math.floor(d.getMonth() / 3) + 1;
      return d.getFullYear() + ' ' + qNum + '분기';
    }

    function btBuildQuarterCard(q) {
      var entries = btEntriesInRange(q.startDate, q.endDate);
      var calc = btCalcPeriod(entries);
      var mTarget = Number(q.monthlyTargetRevenue) || 0;
      var t = today();
      var inRange = q.startDate <= t && q.endDate >= t;
      var pct = mTarget > 0 && inRange ? Math.min(100, Math.round(calc.revenue / (_btElapsedMonths(q.startDate, t) * mTarget) * 100)) : 0;
      var autoName = btQuarterAutoName(q.startDate);
      var h = '<div class="bt-quarter-card' + (inRange ? ' active-quarter' : '') + '">';
      h += '<div class="bt-quarter-header">';
      h += '<span class="bt-quarter-name">' + escapeHtml(autoName) + (inRange ? ' <span class="bt-badge">현재</span>' : '') + '</span>';
      h += '<div class="bt-item-actions"><button class="btn-icon" onclick="btStartEditQuarter(\'' + q.id + '\')">✏️</button><button class="btn-icon" onclick="btDeleteQuarter(\'' + q.id + '\')">🗑️</button></div>';
      h += '</div>';
      h += '<div class="bt-quarter-meta">' + q.startDate + ' ~ ' + q.endDate + '</div>';
      if (q.objective) h += '<div class="bt-quarter-obj">' + escapeHtml(q.objective) + '</div>';
      h += '<div class="bt-quarter-nums">';
      h += '<span>월 목표 ' + btFmtW(mTarget) + '</span>';
      h += '<span>누적 매출 ' + btFmtW(calc.revenue) + '</span>';
      if (inRange) h += '<span>목표 대비 ' + pct + '%</span>';
      h += '</div>';
      if (inRange && mTarget > 0) {
        h += '<div class="bt-progress-wrap"><div class="bt-progress-bar" style="width:' + pct + '%"></div></div>';
      }
      h += '</div>';
      return h;
    }
    function btBuildQuarterForm(q) {
      var def = q ? null : btNextQuarterDefaults();
      var defStart = q ? q.startDate : (def ? def.start : '');
      var autoName = defStart ? btQuarterAutoName(defStart) : (q ? btQuarterAutoName(q.startDate) : '');
      var h = '<div class="bt-item-form">';
      h += '<div class="bt-form-row"><label>분기 이름 <span style="font-weight:400;font-size:11px;color:var(--text-secondary,#999);">(시작일로 자동 결정)</span></label>';
      h += '<div class="bt-quarter-name-preview" id="btQNamePreview">' + escapeHtml(autoName || '시작일을 입력하면 자동 표시됩니다') + '</div>';
      h += '</div>';
      h += '<div class="bt-form-2col">';
      h += '<div class="bt-form-row"><label>시작일</label><input type="date" id="btQStart" class="bt-input" value="' + defStart + '" onchange="var p=document.getElementById(\'btQNamePreview\');if(p)p.textContent=btQuarterAutoName(this.value)||\'\'"></div>';
      ...
      h += '<div class="bt-form-row"><label>목표 / 다짐 <span style="font-weight:400;font-size:11px;color:var(--text-secondary,#999);">(Objective — 선택)</span></label>';
      h += '<textarea id="btQObjective" class="bt-input" rows="3" placeholder="이 분기에 이루고 싶은 것, OKR Objective, 다짐 등을 자유롭게 써주세요" style="resize:vertical;font-size:13px;">' + escapeHtml(q ? (q.objective || '') : '') + '</textarea>';
      h += '</div>';
    ...
    function btNextQuarterDefaults() {
      ...
      if (qs.length > 0) {
        ...
        return { start: fmt(nextStart), end: fmt(nextEnd) };
      }
      var now = new Date();
    ...
    function btSaveQuarter(id) {
      var startDate = (document.getElementById('btQStart') || {}).value || '';
      if (!startDate) { showAlert('입력 오류', '시작일을 입력해주세요.'); return; }
      var q = {
        id: id || btGenId('btq'),
        name: btQuarterAutoName(startDate),  // 자동 생성
        startDate: startDate,
        endDate: (document.getElementById('btQEnd') || {}).value || '',
        monthlyTargetRevenue: parseInt((document.getElementById('btQTarget') || {}).value || '0', 10) || 0,
        objective: ((document.getElementById('btQObjective') || {}).value || '').trim()
      };
      if (!btConfig.quarters) btConfig.quarters = [];
      if (id) {
        var idx = btConfig.quarters.findIndex(function(x) { return x.id === id; });
        if (idx >= 0) btConfig.quarters[idx] = q; else btConfig.quarters.push(q);
      } else {
        btConfig.quarters.push(q);
      }
      btAddingQuarter = false; btEditQuarterId = null;
    ...
    function btBuildSettings() {
      ...
      cats.forEach(function(cat) {
        if (btEditCatId === cat.id) {
          h += '<div class="bt-item-grid-span">' + btBuildCatForm(cat) + '</div>';
        } else {
          h += btBuildCatRow(cat);
        }
      });
      ...
      fcs.forEach(function(fc) {
        if (btEditCostId === fc.id) {
          h += '<div class="bt-item-grid-span">' + btBuildCostForm(fc) + '</div>';
        } else {
          h += btBuildCostRow(fc);
        }
      });
    ...
    function btBuildCatRow(cat) {
      ...
      return '<div class="bt-cat-settings-card">' +
        ...
        '</div>' +
        '<div class="bt-item-actions"><button class="btn-icon" onclick="btStartEditCat(\'' + cat.id + '\')">✏️</button><button class="btn-icon" onclick="btDeleteCat(\'' + cat.id + '\')">🗑️</button></div>' +
        '</div>';
    }

    function btBuildCatForm(cat) {
      var rate = cat && cat.defaultMarginRate != null ? Math.round(cat.defaultMarginRate * 100) : '';
      var h = '<div class="bt-item-form">';
      h += '<div class="bt-form-row"><label>카테고리 이름</label>';
      h += '<div style="display:flex;gap:8px;align-items:center;">';
      h += '<button class="bt-cat-icon-btn" title="아이콘 변경" onclick="btPickCatIcon(\'' + escapeHtml(cat ? cat.icon : '') + '\')">';
      h += '<span id="btCatIconDisplay">' + (cat && cat.icon ? cat.icon : '📦') + '</span>';
      h += '</button>';
      h += '<input type="hidden" id="btCatIconVal" value="' + escapeHtml(cat ? cat.icon : '📦') + '">';
      h += '<input type="text" id="btCatName" class="bt-input" style="flex:1;" placeholder="카테고리 이름" value="' + escapeHtml(cat ? cat.name : '') + '">';
      h += '</div></div>';
      h += '<div class="bt-form-2col">';
      h += '<div class="bt-form-row"><label>단위 라벨</label><input type="text" id="btCatUnit" class="bt-input" placeholder="이용자, 참가자, 구매건수..." value="' + escapeHtml(cat ? cat.unitLabel : '') + '"></div>';
      h += '<div class="bt-form-row"><label>색상</label><input type="color" id="btCatColor" class="bt-input-color" value="' + (cat ? cat.color : '#888888') + '"></div>';
      h += '</div>';
      // 마진율 설명 박스
      ...
      h += '<input type="number" id="btCatRate" class="bt-input" min="0" max="100" placeholder="예: 27" value="' + rate + '">';
      h += '</div>';
      h += '<div class="bt-form-actions"><button class="btn-confirm" onclick="btSaveCat(\'' + (cat ? cat.id : '') + '\')">저장</button><button class="btn-cancel" onclick="btCancelCat()">취소</button></div>';
      h += '</div>';
      return h;
    }

    function btPickCatIcon(current) {
      openEmojiPicker(current || '', function(emoji) {
        var display = document.getElementById('btCatIconDisplay');
        var val = document.getElementById('btCatIconVal');
        if (display) display.textContent = emoji;
        if (val) val.value = emoji;
      });
    }

    function btStartAddCat() { btAddingCat = true; btEditCatId = null; btRenderCurrentTab(); }
    function btStartEditCat(id) { btEditCatId = id; btAddingCat = false; btRenderCurrentTab(); }
    ...
    function btSaveCat(id) {
      ...
      var iconVal = document.getElementById('btCatIconVal');
      var cat = {
        id: id || btGenId('btcat'),
        name: name.trim(),
        icon: (iconVal ? iconVal.value : '') || '📦',
        color: (document.getElementById('btCatColor') || {}).value || '#888888',
        unitLabel: ((document.getElementById('btCatUnit') || {}).value || '건').trim(),
        defaultMarginRate: (isNaN(marginRate) || marginRate === null) ? null : marginRate
      };
    ...
    function btBuildCostRow(fc) {
      return '<div class="bt-cost-row">' +
        '<span class="bt-cat-name">' + escapeHtml(fc.name) + '</span>' +
        '<span class="bt-cost-amount">' + btFmtW(fc.amount) + '/월</span>' +
        '<div class="bt-item-actions"><button class="btn-icon" onclick="btStartEditCost(\'' + fc.id + '\')">✏️</button><button class="btn-icon" onclick="btDeleteCost(\'' + fc.id + '\')">🗑️</button></div>' +
        '</div>';
    }
    function btBuildCostForm(fc) {
      var h = '<div class="bt-item-form">';
      h += '<div class="bt-form-2col">';
      h += '<div class="bt-form-row"><label>항목명</label><input type="text" id="btFcName" class="bt-input" placeholder="임대료, 관리비..." value="' + escapeHtml(fc ? fc.name : '') + '"></div>';
      h += '<div class="bt-form-row"><label>월 금액 (원)</label><input type="number" id="btFcAmount" class="bt-input" min="0" value="' + (fc ? fc.amount : '') + '"></div>';
      h += '</div>';
      h += '<div class="bt-form-actions"><button class="btn-confirm" onclick="btSaveCost(\'' + (fc ? fc.id : '') + '\')">저장</button><button class="btn-cancel" onclick="btCancelCost()">취소</button></div>';
      h += '</div>';
      return h;
    }
    function btStartAddCost() { btAddingCost = true; btEditCostId = null; btRenderCurrentTab(); }
    function btStartEditCost(id) { btEditCostId = id; btAddingCost = false; btRenderCurrentTab(); }
    ...
    function btSaveCost(id) {
      ...
      var fc = {
        id: id || btGenId('btfc'),
        name: name.trim(),
        amount: parseInt((document.getElementById('btFcAmount') || {}).value || '0', 10) || 0
      };
      if (!btConfig.fixedCosts) btConfig.fixedCosts = [];
      if (id) {
        var idx = btConfig.fixedCosts.findIndex(function(f) { return f.id === id; });
        if (idx >= 0) btConfig.fixedCosts[idx] = fc; else btConfig.fixedCosts.push(fc);
      } else {
        btConfig.fixedCosts.push(fc);
      }
      btAddingCost = false; btEditCostId = null;
    ...
    function renderWorkBasketTab() {
      ...
      if (rawTotal === 0) {
        ...
        var allPageIds = basketItems.map(function(it) { return it.id; });
        var allSel = allPageIds.length > 0 && allPageIds.every(function(id) { return basketSelected.indexOf(id) >= 0; });
        var selCount = basketSelected.length;
        html += '<div class="basket-select-bar" style="margin-bottom:8px;">';
        html += '<label class="basket-select-all-label"><input type="checkbox" id="basketSelectAll"' + (allSel ? ' checked' : '') + ' onchange="basketToggleAll(this.checked)"> 전체 선택</label>';
        if (selCount > 0) {
          html += '<span class="basket-selected-count">' + selCount + '개 선택</span>';
          if (selCount === 1) html += '<button class="btn-cancel" style="padding:3px 8px;font-size:12px;" onclick="basketEditSelected()">&#9999; 수정</button>';
          html += '<button class="btn-cancel" style="padding:3px 8px;font-size:12px;color:#e53e3e;" onclick="basketDeleteSelected()">&#128465; 삭제</button>';
          html += '<button class="btn-cancel" style="padding:3px 8px;font-size:12px;" onclick="basketClearSelect()">취소</button>';
        }
        html += '</div>';
        ...
        if (basketItems.length === 0) {
          ...
          basketItems.forEach(function(item) {
            var status = getWorkStatus(item);
            var isSel = basketSelected.indexOf(item.id) >= 0;
            html += '<div class="basket-grid-card' + (isSel ? ' selected' : '') + '" onclick="basketGridToggleSelect(\'' + item.id + '\')">';
            html += '<input type="checkbox" class="basket-item-check basket-grid-check" value="' + item.id + '"' + (isSel ? ' checked' : '') + ' onclick="event.stopPropagation()" onchange="basketUpdateCheck(this)">';
    ...
    function basketToggleAll(checked) {
      var c = document.getElementById('workViewContent');
      if (!c) return;
      c.querySelectorAll('.basket-item-check').forEach(function(cb) {
        cb.checked = checked;
        var id = cb.value;
        var idx = basketSelected.indexOf(id);
        if (checked && idx < 0) basketSelected.push(id);
        else if (!checked && idx >= 0) basketSelected.splice(idx, 1);
      });
      renderWorkBasketTab();
    }

    function basketGridToggleSelect(id) {
      var idx = basketSelected.indexOf(id);
      if (idx >= 0) basketSelected.splice(idx, 1);
      else basketSelected.push(id);
      renderWorkBasketTab();
    }

    function basketUpdateCheck(cb) {
      var id = cb.value;
      var idx = basketSelected.indexOf(id);
      if (cb.checked && idx < 0) basketSelected.push(id);
      else if (!cb.checked && idx >= 0) basketSelected.splice(idx, 1);
      renderWorkBasketTab();
    }

    function basketClearSelect() {
      basketSelected = [];
      renderWorkBasketTab();
    }

    function basketGetSelectedIds() {
      return basketSelected.slice();
    }

    function basketEditSelected() {
      if (basketSelected.length !== 1) return;
      showEditWorkItemModal(basketSelected[0]);
    }

    function basketDeleteSelected() {
      if (basketSelected.length === 0) return;
      var names = basketSelected.map(function(id) {
